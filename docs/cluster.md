...
> **Status:** M0 through M4 shipped on `main`. Cluster forms, gossips, elects a metadata Raft leader, routes session traffic via consistent hashing with transparent SDK redirect, **and replicates every shard write to all alive followers under `acks=quorum` semantics.** Killing a non-leader loses zero messages.
